...
# 前向传播
def forward_propagation(file_path1,file_path2,file_path3,file_path4,file_path5):
    
    img = load_image(file_path1, 784, 1)  # 图像文件路径
    # 步骤 1: 找出 img 中非零的索引
    non_zero_indices = [i for i in range(784) if img[i][0] != 0]
    del img 
    # 步骤 2: 提取 w_i_h 中与非零元素对应的列
    # 这里我们保留 w_i_h 中对应的列
    w_i_h = load_matrix(file_path2, 20, 784)  # w_i_h 矩阵
    ...
    b_i_h = load_matrix(file_path3, 20, 1)  # b_i_h 矩阵
    ...
    w_h_o = load_matrix(file_path4, 10, 20)  # w_i_h 矩阵
    b_h_o = load_matrix(file_path5, 10, 1)  # b_h_o 矩阵
    # 计算 o_pre
...
if __name__ == "__main__": 
    # 前向传播
    o = forward_propagation('img.bin','w_i_h.bin','b_i_h.bin','w_h_o.bin','b_h_o.bin')
